...
class BCNet(nn.Module):
    ...
    def forward(self, v, q):
        ...
        # low-rank bilinear pooling using einsum
            ...
            v_ = nn.Dropout(0.5)(self.v_net(v))
            q_ = self.q_net(q)
    ...
    def forward_with_weights(self, v, q, w):
        v_ = nn.Dropout(0.2)(self.v_net(v)) # b x v x d
...
if __name__=='__main__':
    glimpse = 8
    net = BCNet(1024,1068,568,glimpse).cuda()
    x = torch.Tensor(512,36,1024).cuda()
    y = torch.Tensor(512,14,1068).cuda()
    w = net.forward(x,y)  # [512, 8, 36, 14]
    print(w.shape)
    for g in range(glimpse):
        # w = torch.Tensor(512,36,14).cuda()
        wi = w[:,g,:,:]
        out = net.forward_with_weights(x,y,wi)
        print(out.shape)  # [512, 568]
